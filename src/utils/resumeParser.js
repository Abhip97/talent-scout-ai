...
export const parseResume = async (resumeText, fileName, apiKey) => {
  const messages = [
  ...
  try {
    const raw = await callGroq(apiKey, messages, 1500);
...
export const parseMultipleResumes = async (files, apiKey, onProgress) => {
  const results = [];
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      const candidate = await parseResume(text, file.name, apiKey);
...
const sanitizeResumeParsed = (parsed) => {
  return {
    ...
    expectedSalaryLPA: {
      min: Number(parsed.expectedSalaryLPA?.min) || 0,
      max: Number(parsed.expectedSalaryLPA?.max) || 0,
    },
